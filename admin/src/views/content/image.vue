...
import { getImageReviewList, reviewImage, batchReviewImages } from '@/api/content'
...
async function handleAudit(id, action) {
  ...
  try {
    await ElMessageBox.confirm('确认通过该图片审核？', '确认操作', {
...
async function handleBatchAudit(action) {
  ...
  try {
    await ElMessageBox.confirm(`确认通过选中的${selectedIds.value.length}张图片？`, '批量审核', {
      confirmButtonText: '确认',
      cancelButtonText: '取消',
      type: 'success'
    })
    await batchReviewImages({ ids: selectedIds.value, action })
...
async function confirmReject() {
  try {
    if (isBatchReject.value) {
      ...
      await reviewImage(currentRejectId.value, {
        action: 'reject',
        remark: rejectReason.value
      })
